...
from airflow.models import DagBag, DagModel, DAG, TaskInstance
from airflow.executors import get_default_executor
from airflow import settings
from airflow.ti_deps.dep_context import DepContext, REQUEUEABLE_DEPS, RUNNING_DEPS, SCHEDULEABLE_STATES, RUNNABLE_STATES
...
apis_metadata = [
    ...
    },
    {
        ...
        "arguments": [
            ...
            {"name": "rerun_failed_tasks", "description": "Rerun failed tasks instead of throwing exception.", "form_input_type": "checkbox", "required": False},
            {"name": "ignore_dependencies", "description": "Ignore task-specific dependencies, e.g. upstream, depends_on_past, and retry delay dependencies", "form_input_type": "checkbox", "required": False},
    ...
    },
    {
        ...
        "airflow_version": "None - Custom API",
        "http_method": "POST",
        "post_body_description": "variables_file - POST Body Element - REQUIRED",
        "form_enctype": "multipart/form-data",
        "arguments": [],
        "post_arguments": [
            {"name": "variables_file", "description": "Json file containing variables", "form_input_type": "file", "required": True},
        ]
    },
    {
        "name": "refresh_dag",
    ...
    {
        "name": "list_dag_runs",
        "description": "List the DAG Runs",
        "airflow_version": "1.10.3 or greater",
        "http_method": ["GET", "POST"],
        "arguments": [
            {"name": "dag_id", "description": "The id of the dag", "form_input_type": "text", "required": True},
            {"name": "start_date", "description": "Override start_date YYYY-MM-DD", "form_input_type": "text", "required": False},
            {"name": "end_date", "description": "Override end_date YYYY-MM-DD", "form_input_type": "text", "required": False},
            {"name": "state", "description": "fetch DAGs with specific state only", "form_input_type": "text", "required": False}
        ]
    },
]
    ...
    def secure_check(arg):
        ...
        if airflow_expected_http_token:
            ...
            if request.headers.get(airflow_rest_api_plugin_http_token_header_name, None) != airflow_expected_http_token:
                ...
                return REST_API_Response_Util.get_403_error_response(base_response=base_response, error=warning_message)
        return func(arg)
...
class REST_API_Response_Util():
    ...
    @staticmethod
    def _get_final_response(base_response, output=None, airflow_cmd=None, http_response_code=None, warning=None, error=None):
    ...
    @staticmethod
    def _get_error_response(base_response, error_code, error=None):
        base_response["status"] = "ERROR"
        return REST_API_Response_Util._get_final_response(base_response=base_response, error=error, http_response_code=error_code), error_code

    # Set the Base Response as a 400 HTTP Response object
    @staticmethod
    def get_400_error_response(base_response, error=None):
        logging.warning("Returning a 400 Response Code with response '" + str(error) + "'")
        return REST_API_Response_Util._get_error_response(base_response, 400, error)

    # Set the Base Response as a 403 HTTP Response object
    @staticmethod
    def get_403_error_response(base_response, error=None):
        logging.warning("Returning a 403 Response Code with response '" + str(error) + "'")
        return REST_API_Response_Util._get_error_response(base_response, 403, error)

    # Set the Base Response as a 500 HTTP Response object
    @staticmethod
    def get_500_error_response(base_response, error=None):
        logging.warning("Returning a 500 Response Code with response '" + str(error) + "'")
        return REST_API_Response_Util._get_error_response(base_response, 500, error)
...
class REST_API(get_baseview()):
    ...
    @staticmethod
    def get_dagbag(collect_all = False):
        load_from_db = get_config_boolean_value('core', 'store_serialized_dags', False)
        dagbag = DagBag(store_serialized_dags=load_from_db)
        if load_from_db and collect_all:
            logging.info("loading dags from db")
            dagbag.collect_dags_from_db()
        return dagbag

    @staticmethod
    def get_argument(request, arg, default=None):
        return request.args.get(arg) or request.form.get(arg) or default
    ...
    if rbac_authentication_enabled == True:
        ...
        def list(self):
            ...
            dagbag = self.get_dagbag(collect_all=True)
            dags = []
            ...
            return self.render_template("/api_plugin/index.html",
                                        dags=dags,
        ...
        def index(self):
            ...
            return self.render("api_plugin/index.html",
                                   dags=dags,
                                   airflow_webserver_base_url=airflow_webserver_base_url,
                                   rest_api_endpoint=rest_api_endpoint,
                                   apis_metadata=apis_metadata,
    ...
    def api(self):
        ...
        dag_bag = self.get_dagbag(collect_all=False)
        if dag_id is not None and dag_bag.get_dag(dag_id) is None:
        ...
        elif api == "import_variables":
            final_response = self.import_variables(base_response)
        elif api == "deploy_dag":
        ...
        elif api == "list_dag_runs":
            final_response = self.dag_runs(base_response)
        elif api == "clear":
            final_response = self.clear_dag(base_response)
        elif api == "backfill":
            final_response = self.backfill(base_response)
        elif api == "list_tasks":
            final_response = self.list_tasks(base_response)
        elif api == "run":
            final_response = self.run(base_response)
        else:
    ...
    def run(self, base_response):
        ...
        if dag is None:
            return REST_API_Response_Util.get_400_error_response(base_response, "invalid dag_id")

        execution_date_iso = self.get_argument(request, 'execution_date')
        execution_date = timezone.parse(execution_date_iso) if execution_date_iso else None
        if not execution_date:
            return REST_API_Response_Util.get_400_error_response(base_response, "execution date is required")

        task_id = self.get_argument(request, 'task_id')
        if self.is_arg_not_provided(task_id):
            return REST_API_Response_Util.get_400_error_response(base_response, "task_id should be provided")

        mark_success = self.get_argument(request, 'mark_success', False)
        ignore_all_dependencies = self.get_argument(request, 'ignore_all_dependencies', False)
        ignore_depends_on_past = self.get_argument(request, 'ignore_depends_on_past', False)
        ignore_dependencies = self.get_argument(request, 'ignore_dependencies', False)
        ignore_ti_state = self.get_argument(request, 'force', False)
        pool = self.get_argument(request, 'pool', 'default_pool')
        ...
        # if dag run is in running state, check if its ready by now
        # scheduler is slow to figure out this of its own in airflow
        dag_run = dag.get_dagrun(execution_date=execution_date)
        if dag_run is not None:
            logging.info("dag run state on {} is in {} state".format(dag_run.execution_date, dag_run.get_state()))
            if dag_run.get_state() == State.RUNNING or dag_run.get_state() == State.FAILED:
                dag_run.dag = dag
                dag_run.update_state()
                logging.info("dag run state on {} is in running state, after update it is in: {}".format(dag_run.execution_date, dag_run.get_state()))

        # basic sanity dependencies
        can_execute_task = (ti.are_dependencies_met(dep_context=DepContext(
                deps={
                    NotInRetryPeriodDep(),
                    PrevDagrunDep(),
                    TriggerRuleDep(),
        ...
        if can_execute_task is False:
            error_message = "basic dependency failed: cannot execute task [depends_on_past, trigger_rull_for_upstream, allready_in_retry, skipped]"
            logging.error(error_message)
            return REST_API_Response_Util.get_500_error_response(base_response, error_message)

        if previous_state not in RUNNABLE_STATES:
            error_message = "not in valid state to run - {}".format(previous_state)
            logging.error(error_message)
            return REST_API_Response_Util.get_500_error_response(base_response, error_message)

        # queue-able dependencies
        dep_context = DepContext(
                ...
                ignore_task_deps=ignore_dependencies,
                ignore_ti_state=ignore_ti_state)
        dependency_met = ti.are_dependencies_met(dep_context=dep_context)

        executor = get_default_executor()
        executor.start()
        logging.info("Sending {} to executor.".format(task_id))
        ...
        # TODO(kush): make this check reliable
        # don't throw error at the moment because of this
        # if dependency_met is False:
        #     error_message = "dependency not met"

        return REST_API_Response_Util.get_200_response(base_response=base_response, output=task_details)
    ...
    def list_tasks(self, base_response):
        ...
        logging.info(tasks)
        return REST_API_Response_Util.get_200_response(base_response=base_response, output=tasks)
    ...
    def backfill(self, base_response):
        ...
        start_date_iso = self.get_argument(request, 'start_date')
        end_date_iso = self.get_argument(request, 'end_date')
        start_date = timezone.parse(start_date_iso) if start_date_iso else None
        end_date = timezone.parse(end_date_iso) if end_date_iso else None

        if not start_date and not end_date:
            return REST_API_Response_Util.get_400_error_response(base_response, "start and end date is required")

        # If only one date is passed, using same as start and end
        end_date = end_date or start_date
        start_date = start_date or end_date

        task_regex = self.get_argument(request, 'task_regex', None)
        dry_run = self.get_argument(request, 'dry_run', False)
        ignore_dependencies = self.get_argument(request, 'ignore_dependencies', False)
        mark_success = self.get_argument(request, 'mark_success', False)
        ignore_first_depends_on_past = self.get_argument(request, 'ignore_first_depends_on_past', False)
        donot_pickle = self.get_argument(request, 'donot_pickle', False)
        reset_dagruns = self.get_argument(request, 'reset_dagruns', False)
        rerun_failed_tasks = self.get_argument(request, 'rerun_failed_tasks', False)
        local = self.get_argument(request, 'local', False)
        run_backwards = self.get_argument(request, 'run_backwards', False)
        pool = self.get_argument(request, 'pool', 'default')
        delay_on_limit = self.get_argument(request, 'delay_on_limit', 1)
        ...
        if dry_run:
            print("Dry run of DAG {0} on {1}".format(dag_id, start_date))
            for task in dag.tasks:
                print("Task {0}".format(task.task_id))
                ti = TaskInstance(task, start_date)
                ti.dry_run()
        else:
            if reset_dagruns:
                DAG.clear_dags(
        ...
        dag.run(
                ...
                mark_success=mark_success,
                local=local,
                donot_pickle=(donot_pickle or
                            get_config_boolean_value('core', 'donot_pickle', True)),
        ...
        res_out = "backfill completed in {} sec".format(time.time() - start_backfill_time)
        return REST_API_Response_Util.get_200_response(base_response=base_response, output=res_out)

    def clear_dag(self, base_response):
        dag_id = self.get_argument(request, 'dag_id')
        ...
        task_regex = self.get_argument(request, 'task_regex')
        upstream = self.get_argument(request, 'upstream', False)
        downstream = self.get_argument(request, 'downstream', False)
        only_failed = self.get_argument(request, 'only_failed', False)
        only_running = self.get_argument(request, 'only_running', False)
        exclude_subdags = self.get_argument(request, 'exclude_subdags', False)
        exclude_parentdag = self.get_argument(request, 'exclude_parentdag', False)

        dags = [self.get_dagbag().get_dag(dag_id)]
        if len(dags) == 0:
            return REST_API_Response_Util.get_400_error_response(base_response, "invalid dag_id")
        ...
        cleared = DAG.clear_dags(
            ...
            include_parentdag=not exclude_parentdag,
        )

        res_out = "{} instances cleared".format(cleared)
        return REST_API_Response_Util.get_200_response(base_response=base_response, output=res_out)

    # Get all the dag runs using airflow's experimental api
    def dag_runs(self, base_response):
        ...
        start_date_iso = request.args.get("start_date")
        end_date_iso = request.args.get("end_date")
        ...
        try:
            ...
            if start_date:
                runs = list(filter(lambda x: start_date <= timezone.parse(x.get("execution_date")), runs))

            if end_date:
                runs = list(filter(lambda x: timezone.parse(x.get("execution_date")) <= end_date, runs))

            response = REST_API_Response_Util.get_200_response(base_response=base_response, output=runs)
        except Exception as e:
            error_message = "An error occurred while trying to list the DAG Runs '{dag_id}': {error}".format(
                dag_id=dag_id,
                error=str(e)
            )
            logging.error(error_message)
            response = REST_API_Response_Util.get_500_error_response(base_response, error_message)
    ...
    def execute_cli(self, base_response, api_metadata):
        ...
        if output.get("stderr"):
            return REST_API_Response_Util.get_500_error_response(base_response, output.get("stderr"))

        return REST_API_Response_Util.get_200_response(base_response=base_response, output=output.get("stdout"), airflow_cmd=airflow_cmd)
    ...
    def import_variables(self, base_response):
        logging.info("Executing custom 'import_variables' function")

        if 'variables_file' not in request.files or request.files['variables_file'].filename == '':  # check if the post request has the file part
            logging.warning("The variables_file argument wasn't provided")
            return REST_API_Response_Util.get_400_error_response(base_response, "variables_file should be provided")

        variables_file = request.files['variables_file']
        variables_dir = str(Path.home()) + '/variables/'
        Path(variables_dir).mkdir(parents=True, exist_ok=True)
        if variables_file:
            try:
                save_file_path = os.path.join(variables_dir, variables_file.filename)
                variables_file.save(save_file_path)
                variables_file.close()
                logging.info("saved imported variables file at: " + save_file_path)

                # validate if file has valid json
                # if we do variables_file.read(), the file automatically gets closed
                # and therefore we won't be able to save it again. for this purpose we're
                # saving the file first, and then reading it again for verification purpose.
                try:
                    with open(save_file_path, 'rb') as fp:
                        json.loads(fp.read()).keys()
                except Exception as e:
                    err = "variables file contains invalid JSON. " + str(e)
                    logging.error(err)
                    return REST_API_Response_Util.get_400_error_response(base_response, err)

                airflow_cmd_split = ["airflow", "variables", "-i",  save_file_path]
                output = self.execute_cli_command(airflow_cmd_split)
                if output.get("stderr"):
                    logging.error(output.get("stderr"))
                    return REST_API_Response_Util.get_500_error_response(base_response, output.get("stderr"))
            except Exception as e:
                err = "Failed to set the variables from file: " + str(e)
                logging.error(err)
                return REST_API_Response_Util.get_400_error_response(base_response, err)
        else:
            err = "variables_file was not provided"
            logging.error(err)
            return REST_API_Response_Util.get_400_error_response(base_response, err)

        return REST_API_Response_Util.get_200_response(base_response=base_response, output="Variables updated from file [{}]".format(variables_file.filename))
...
if rbac_authentication_enabled == True:
    rest_api_view = {"category" : "Admin", "name" : "REST API",  "view": REST_API()}
else:
